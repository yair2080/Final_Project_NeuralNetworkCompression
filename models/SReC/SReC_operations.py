...
def timeTest_DeAndCompress(connectionToFather, path ,load,
    ...
    """
def TrainSReC( conn,train_path: str, eval_path: str, train_file, eval_file,
...
def TrainSReC_casing(conn,
    ...
    retDict=prepFolders(trainFolder=train_path,validFolder=eval_path)
    
    with open(retDict["trainFile"], "r") as trainFile:  
        with open(retDict["validFile"], "r") as validFile:  
            TrainSReC( conn=conn,train_path=train_path, eval_path=eval_path, train_file=trainFile, eval_file=validFile,
