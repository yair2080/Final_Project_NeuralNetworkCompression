...
class MyImageFolder(data.Dataset):
    ...
    def __init__(self,
        ...
        filenames=[]
        import ntpath
        for path in filenamesPaths:
            tempFileName=ntpath.basename(path)
            filenames.append(tempFileName)
            fileNameDict[tempFileName]=path
    ...
    def read(self, filename: str) -> bytes:
        self.dir_path=os.path.split(self.fileNameDict[filename])[0]
        with open(self.fileNameDict[filename], "rb") as f:
            return f.read()
...
class MySignalFolder(data.Dataset):
    """ Generic Dataset class for a directory full of signals given 
        a list of signals filenames. Can be used for any unsupervised 
        learning task without labels.
    """
    ...
    def __init__(self,
                 filenamesPaths: List[str],
                 scale: int,
                 transforms =None,  # torchvision Transform
                 ) -> None:
        """ param dir_path: Signal dir
            param filenames: List of image filenames in the directory.
            param transforms: a torchvision Transform that can be applied to 
                the image.
        """
    ...
    def to_tensor_not_normalized(self, signal) -> torch.Tensor:
    ...
    def read(self, filename: str) -> bytes:
        originalArray=(scipy.io.loadmat(self.fileNameDict[filename]))["val"]
        asTensor=torch.Tensor(originalArray[None, :, :])
        if(asTensor.size()!=(1,12,5000)):
            asTensor=np.reshape(np.resize(asTensor,12*5000),(1,12,5000))
        return torch.Tensor(asTensor)
