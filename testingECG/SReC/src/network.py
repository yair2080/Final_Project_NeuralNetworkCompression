...
class PixDecoder(nn.Module):
    ...
    def __init__(self, scale: int) -> None:
        super().__init__()
        self.loss_fn = lm.DiscretizedMixLogisticLoss(rgb_scale=False)
        self.scale = scale
...
class StrongPixDecoder(PixDecoder):
    def __init__(self, scale: int) -> None:
        ...
        self.mix_logits_prob_clf = nn.ModuleList([
            prob_clf.AtrousProbabilityClassifier(
                configs.n_feats, C=1, K=configs.K,
                num_params=self.loss_fn._num_params)
            for _ in range(1, 4)
        ])
        self.feat_convs = nn.ModuleList([
            util.conv(configs.n_feats, configs.n_feats, 1) for _ in range(1, 4)
        ])
    ...
    def forward_probs(
        ...
        for i, (rgb_dec, clf, feat_conv) in enumerate(
            ...
            probs = clf(z)
            lower = torch.max(
                pix_sum - (1 - i) * 255, torch.tensor(0., device=x.device))
            upper = torch.min(
